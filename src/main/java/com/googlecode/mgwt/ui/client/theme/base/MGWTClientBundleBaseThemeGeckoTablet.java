...
/**
 * The geckotablet client bundle
 * 
 * @author Daniel Kurka
 * 
 */
public interface MGWTClientBundleBaseThemeGeckoTablet extends ClientBundle, MGWTClientBundle {

	@Source({ "css/groupinglist.css", "css/geckotablet/groupinglist.css" })
	GroupingList getGroupingList();

//	@Source({ "css/progressbar.css", "css/geckotablet/progressbar.css" })
...
//	@Source({ "css/progressindicator.css", "css/geckotablet/progressindicator.css" })
//	ProgressIndicatorCss getProgressIndicatorCss();

	@Source({ "css/header.css", "css/geckotablet/header.css" })
	HeaderCss getHeaderCss();

//	@Source({ "css/slider.css", "css/geckotablet/slider.css" })
//	SliderCss getSliderCss();

	@Source({ "css/carousel.css", "css/geckotablet/carousel.css" })
	CarouselCss getCarouselCss();

	@Source({ "css/list.css", "css/geckotablet/list.css" })
	ListCss getListCss();

	@Source("resources/list/arrow.png")
	DataResource listArrow();

	@Source({ "css/searchbox.css", "css/geckotablet/searchbox.css" })
	...
	@Source({ "css/buttons.css", "css/geckotablet/buttons.css" })
	ButtonCss getButtonCss();

	@Source({ "css/scrollpanel.css", "css/geckotablet/scrollpanel.css" })
	ScrollPanelCss getScrollPanelCss();

	@Source({ "css/buttonbar.css", "css/geckotablet/buttonbar.css" })
	ButtonBarCss getButtonBarCss();

	@Source({ "css/dialog.css", "css/geckotablet/dialog.css" })
	DialogCss getDialogCss();

	@Source({ "css/main.css", "css/geckotablet/main.css" })
	MainCss getMainCss();

	@Source({ "css/input.css", "css/geckotablet/input.css" })
	...
	@Source({ "css/pulltorefresh.css", "css/geckotablet/pulltorefresh.css" })
	PullToRefreshCss getPullToRefreshCss();

	@Source({ "css/tabbar.css", "css/geckotablet/tabbar.css" })

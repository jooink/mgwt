...
/**
 * The geckomobile without retina client bundle
 * 
 * @author Daniel Kurka
 * 
 */
public interface MGWTClientBundleBaseThemeGeckoMobile extends ClientBundle, MGWTClientBundle {

	@Source({ "css/groupinglist.css", "css/geckomobile/groupinglist.css" })
	GroupingList getGroupingList();

//	@Source({ "css/progressbar.css", "css/geckomobile/progressbar.css" })
...
//	@Source({ "css/progressindicator.css", "css/geckomobile/progressindicator.css" })
//	ProgressIndicatorCss getProgressIndicatorCss();

	@Source({ "css/header.css", "css/geckomobile/header.css" })
	HeaderCss getHeaderCss();

//	@Source({ "css/slider.css", "css/geckomobile/slider.css" })
//	SliderCss getSliderCss();

	@Source({ "css/carousel.css", "css/geckomobile/carousel.css" })
	CarouselCss getCarouselCss();

	@Source({ "css/list.css", "css/geckomobile/list.css" })
	ListCss getListCss();

	@Source("resources/list/arrow.png")
	DataResource listArrow();

	@Source({ "css/searchbox.css", "css/geckomobile/searchbox.css" })
	...
	@Source({ "css/buttons.css", "css/geckomobile/buttons.css" })
	ButtonCss getButtonCss();

	@Source({ "css/scrollpanel.css", "css/geckomobile/scrollpanel.css" })
	ScrollPanelCss getScrollPanelCss();

	@Source({ "css/buttonbar.css", "css/geckomobile/buttonbar.css" })
	ButtonBarCss getButtonBarCss();

	@Source({ "css/dialog.css", "css/geckomobile/dialog.css" })
	DialogCss getDialogCss();

	@Source({ "css/main.css", "css/geckomobile/main.css" })
	MainCss getMainCss();

	@Source({ "css/input.css", "css/geckomobile/input.css" })
	...
	@Source({ "css/pulltorefresh.css", "css/geckomobile/pulltorefresh.css" })
	PullToRefreshCss getPullToRefreshCss();

	@Source({ "css/tabbar.css", "css/geckomobile/tabbar.css" })
